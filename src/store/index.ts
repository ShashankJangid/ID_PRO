  ...
  persist(
    (set, get) => ({
      ...
      setCardDataList: (list) =>
        set({ cardDataList: list, activeCardIndex: 0 }),
      addCardData: (data) =>
        set((state) => ({
          cardDataList: [...state.cardDataList, data],
        })),
      updateActiveCard: (data) =>
        set((state) => {
          const list = [...state.cardDataList];
          if (list[state.activeCardIndex]) {
            list[state.activeCardIndex] = {
              ...list[state.activeCardIndex],
              ...data,
            };
          }
...
// ─── Helper: Get data field value from card data ───
...
// ─── Helper: Suggest column mappings ───
export function suggestMappings(
  ...
  const fieldPatterns: Record<DataField, string[]> = {
    ...
    code: ['code', 'emp code', 'employee code', 'roll no', 'rollno', 'id', 'id no', 'roll number', 'emp. code', 'registration'],
